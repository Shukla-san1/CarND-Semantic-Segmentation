...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    train_op = tf.placeholder(tf.float32, name='train_op')
    cross_entropy_loss = tf.placeholder(tf.float32, name='cross_entropy_loss')
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for i in range(epochs):
        for image,label in get_batches_fn(batch_size):
            ...
            if(i==2):
                break
            #sess.run(train_op, feed_dict={input_image: images, correct_label: gt_image,keep_prob :0.60})
            #_,training_loss = sess.run(train_op, feed_dict={input_image:image, correct_label:label, keep_prob: 0.70, learning_rate:  0.001})
...
def run():
    ...
    #input_image = tf.placeholder(tf.float32, (None, 160, 576, 3))
    correct_label = tf.placeholder(tf.float32, [None, 160, 576, num_classes])
...
def pipeline():
    global sess,logits,keep_prob,input_image,image_shape
    num_classes = 2
    epochs = 20
    batch_size = 32
    learning_rate = .001
    ...
    #keep_prob = tf.placeholder(tf.float32, name='keep_prob')
    #layer3_out = tf.placeholder(tf.float32, [None, None, None, 256])
    #layer4_out = tf.placeholder(tf.float32, [None, None, None, 512])
    #layer7_out = tf.placeholder(tf.float32, [None, None, None, 4096])
    #logits = tf.placeholder(tf.float32, name='logits')
    #op = tf.placeholder(tf.float32, name='op')
    #train_op = tf.placeholder(tf.float32, name='train_op')
    #cross_entropy_loss = tf.placeholder(tf.float32, name='cross_entropy_loss')
    #graph = tf.Graph()
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, "./tmp/model.ckpt")
       #helper.save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, keep_prob, input_image)
        #return helper.gen_video_output(sess, logits, keep_prob,input_image, image, image_shape)
...
#clip = VideoFileClip('driving.mp4')
#new_clip = clip.fl_image(test_pipeline)
